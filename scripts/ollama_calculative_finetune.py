...
class OllamaCalculativeFineTuner:
    ...
    def test_model_response(self, test_prompt):
        ...
        try:
            payload = {"model": self.model_name, "prompt": test_prompt, "stream": False}
    ...
    def simulate_calculative_training(self, dataset_file):
        ...
        print(
            f"🧪 Initial model test: '{test_prompt}' -> '{initial_response[:100]}...'"
        )
        ...
        for phase_name, phase_config in self.PHASE_CONFIG.items():
            ...
            phase_examples = examples[
                trained_examples : trained_examples + phase_config["examples_per_phase"]
            ]
        ...
        training_log = {
            ...
            "initial_test": {"prompt": test_prompt, "response": initial_response},
        ...
        log_file = (
            ...
            / f"calculative_training_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--subject", default="mathematics", help="Subject specialization"
    )
    ...
    tuner = OllamaCalculativeFineTuner(model_name=args.model, subject=args.subject)
    ...
    print("   2. Analyze model responses across phases")
    print("   3. Consider actual fine-tuning implementation")
