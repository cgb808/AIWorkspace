...
import argparse, json, random, re, hashlib, statistics
...
class FamilyTutorCurator:
    ...
    def add_example(self, user_msg: str, assistant_msg: str, source: str, 
                   category: str, synthetic: bool = False) -> bool:
        """Add example if it passes quality filters"""
        self.stats['total_processed'] += 1
        
        # Content filtering
        if has_inappropriate_content(user_msg) or has_inappropriate_content(assistant_msg):
            self.stats['rejected_inappropriate'] += 1
            return False
        
        # Length filtering
        assistant_words = len(assistant_msg.split())
        if assistant_words < MIN_ASSISTANT_WORDS or assistant_words > MAX_ASSISTANT_WORDS:
    ...
    def generate_educational_examples(self, count: int = 100) -> None:
        ...
        educational_qa = [
            ("What is photosynthesis?", "Photosynthesis is how plants make their own food! They use sunlight, water, and air (carbon dioxide) to create sugar, which gives them energy to grow. It's like plants have their own kitchen powered by the sun!"),
            ("Why is the sky blue?", "The sky looks blue because of how sunlight bounces around in our atmosphere! Sunlight has many colors mixed together, but blue light gets scattered more than other colors, so that's what we see most."),
            ("How do birds fly?", "Birds can fly because they have special lightweight bones, powerful flight muscles, and feathers that help them catch air. Their wings are shaped to create lift - the same force that helps airplanes fly!"),
            ("What makes rainbows?", "Rainbows happen when sunlight shines through water droplets in the air! The water acts like a prism and splits the white sunlight into all its beautiful colors - red, orange, yellow, green, blue, indigo, and violet."),
            ("Why do we need to sleep?", "Sleep is super important because it's when our bodies and brains recharge! While we sleep, our brains organize memories, our bodies grow and repair themselves, and we get energy for the next day."),
            ("How do magnets work?", "Magnets have invisible force fields around them! Some materials like iron are attracted to these fields, while others aren't. The Earth is like a giant magnet too, which is how compasses work!"),
            ("What are the phases of the moon?", "The moon looks different throughout the month because of how sunlight hits it! Sometimes we see the whole bright side (full moon), sometimes just a sliver (crescent), and sometimes we can't see it at all (new moon)."),
            ("How do fish breathe underwater?", "Fish breathe through special organs called gills! Water flows over their gills, which take oxygen from the water instead of from air like we do. It's like having underwater lungs!"),
            ("Why do leaves change color in fall?", "Leaves change color because they stop making the green stuff (chlorophyll) that helps them make food. When the green fades away, we can see the other colors that were hiding underneath - yellow, orange, and red!"),
            ("What is gravity?", "Gravity is an invisible force that pulls things toward each other! The bigger something is, the stronger its gravity. Earth's gravity keeps us on the ground and makes things fall down instead of floating away!")
        ]
        
        for i in range(min(count, len(educational_qa) * 3)):  # Allow repetition with variation
...
def main():
    ...
    parser.add_argument('--output', default='data/family_tutor/curated_family_tutor.jsonl',
                       help='Output JSONL file path')
    parser.add_argument('--max-examples', type=int, default=4000,
                       help='Maximum total examples to generate')
    parser.add_argument('--seed', type=int, default=42,
                       help='Random seed for reproducibility')
    parser.add_argument('--eli5-count', type=int, default=1000,
                       help='Maximum ELI5 examples to include')
    parser.add_argument('--safety-count', type=int, default=50,
                       help='Number of safety examples to generate')
    parser.add_argument('--educational-count', type=int, default=200,
