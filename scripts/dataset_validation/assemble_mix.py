...
class DatasetBalancer:
    ...
    def analyze_current_distribution(self, examples: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def stratified_sample(self, examples_by_role: Dict[str, List[Dict[str, Any]]], 
                         target_counts: Dict[str, int]) -> List[Dict[str, Any]]:
        ...
        for role, target_count in target_counts.items():
            ...
            if current_count <= target_count:
                ...
                if current_count < target_count:
                    print(f"Note: Role '{role}' has {current_count} examples, need {target_count}")
                    ...
                    self.stats["sampling_actions"][role] = f"upsampled_from_{current_count}_to_{target_count}"
                else:
                ...
                self.stats["sampling_actions"][role] = f"downsampled_from_{current_count}_to_{target_count}"
            
            balanced_examples.extend(selected)
        
        return balanced_examples
    
    def balance_dataset(self, input_file: str, output_file: str, 
                       target_size: int | None = None) -> Dict[str, Any]:
        ...
        for role, count in sorted(final_dist.items()):
            percentage = count / len(balanced_examples) * 100 if balanced_examples else 0
            print(f"  {role}: {count} ({percentage:.1f}%)")
    ...
    def create_train_val_split(self, input_file: str, 
                              train_file: str, 
                              val_file: str, 
        ...
        for role in examples_by_role.keys():
            train_role_count = sum(1 for ex in train_examples 
                                 if ex.get("meta", {}).get("role") == role)
            val_role_count = sum(1 for ex in val_examples 
                               if ex.get("meta", {}).get("role") == role)
...
def main():
    if len(sys.argv) < 3:
        ...
        print("   or: python assemble_mix.py --split <input_file> <train_file> <val_file> [val_ratio]")
        sys.exit(1)
    
    if sys.argv[1] == "--split":
        if len(sys.argv) < 5:
            print("Usage for split: python assemble_mix.py --split <input_file> <train_file> <val_file> [val_ratio]")
            sys.exit(1)
        ...
        split_stats = balancer.create_train_val_split(input_file, train_file, val_file, val_ratio)
