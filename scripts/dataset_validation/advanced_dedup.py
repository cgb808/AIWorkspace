...
class AdvancedDeduplicator:
    def __init__(self, 
        ...
        self.location_patterns = [
            re.compile(r'\b\d{1,5}\s+[A-Z][a-z]+\s+(Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln)\b'),
    ...
    def check_large_tutor_excess(self, example: Dict[str, Any], role_medians: Dict[str, float]) -> bool:
    ...
    def generate_report(self) -> str:
        ...
        for role, stats in sorted(self.stats["role_stats"].items()):
            ...
            lines.append(f"  {role}: {stats['count']:,} ({percentage:.1f}%) - avg {stats['avg_tokens']:.1f} tokens")
