...
class AlpacaSchemaValidator:
    ...
    def _update_stats(self, example: Dict[str, Any]):
        ...
        # Role distribution
        self.stats["role_distribution"][role] = (
            self.stats["role_distribution"].get(role, 0) + 1
        )
        ...
        self.stats["length_stats"][role]["instruction_tokens"].append(
            instruction_tokens
        )
        self.stats["length_stats"][role]["output_tokens"].append(output_tokens)
        self.stats["length_stats"][role]["total_tokens"].append(
        ...
        for sep in separators_used:
            self.stats["separator_coverage"][sep] = (
                self.stats["separator_coverage"].get(sep, 0) + 1
            )
    ...
    def generate_report(self) -> str:
        ...
        if self.stats["role_distribution"]:
            ...
            for role, count in sorted(self.stats["role_distribution"].items()):
                ...
                report.append(
                    f"  {role}: {count} ({percentage:.1f}%) {status} [target: {target}%]"
                )
            report.append("")
        ...
        if self.stats["length_stats"]:
            ...
            for role, stats in self.stats["length_stats"].items():
                ...
                report.append(
                    f"    Instruction tokens: avg={stats.get('instruction_tokens_avg', 0):.1f}, "
                    f"min={stats.get('instruction_tokens_min', 0)}, "
                    f"max={stats.get('instruction_tokens_max', 0)}"
                )
                report.append(
                    f"    Output tokens: avg={stats.get('output_tokens_avg', 0):.1f}, "
