...
def compute_confidence(
    reading: float, word_count: int, is_fallback: bool, synthetic: bool
) -> float:
...
def process(
    ...
    def run_once(examples_iter, cfg):
        ...
        for ex in examples_iter:
            ...
            if reading < cfg["min_reading"]:
                rejections["low_readability"] = rejections.get("low_readability", 0) + 1
                continue
            synthetic = bool(meta.get("synthetic"))
            if synthetic and cfg["exclude_synthetic"]:
                rejections["synthetic"] = rejections.get("synthetic", 0) + 1
                continue
            ...
            if is_fallback:
                ...
                if (
                    projected_total > 0
                    and (fallback_kept + 1) / projected_total
                    > cfg["max_fallback_fraction"]
                ):
                    rejections["fallback_cap"] = rejections.get("fallback_cap", 0) + 1
                    continue
            ...
            if sidecar_dir:
                if sidecar_mode == "files":
                    safe_write_json(
                        enriched_meta, os.path.join(sidecar_dir, f"{h}.meta.json")
                    )
                else:
            ...
            for idx, atext in enumerate(assistant_texts):
                ...
                for c, e in zip(
                    chunks, embeddings if embeddings else [[0.0]] * len(chunks)
                ):
        ...
        summary = {
            ...
            "fallback_fraction": (
                round(fallback_kept / max(accepted, 1), 4) if accepted else 0.0
            ),
            "length_avg": round(sum(lengths) / len(lengths), 2) if lengths else 0,
            "length_median": statistics.median(lengths) if lengths else 0,
            "reading_mean": (
                round(sum(reading_vals) / len(reading_vals), 2) if reading_vals else 0
            ),
        ...
        if sidecar_dir and sidecar_mode == "aggregate":
            safe_write_json(
                {"batch_tag": batch_tag, "source": source, "items": sidecar_agg},
                os.path.join(sidecar_dir, f"{batch_tag}_sidecars.json"),
            )
        return summary
    ...
    while True:
        ...
        print(
            f"[quality] Attempt {attempt} -> accepted {summary['accepted']} / {summary['scanned']} (rate {summary['accept_rate']})"
        )
        if min_accepted and summary["accepted"] < min_accepted:
            # Relax thresholds
            cfg["min_reading"] = max(0, cfg["min_reading"] - relax_step)
            cfg["max_fallback_fraction"] = min(
                1.0, cfg["max_fallback_fraction"] + relax_step / 100
            )
            cfg["min_words"] = max(3, math.floor(cfg["min_words"] * 0.9))
            print(
                f"[quality] Relaxing constraints -> min_reading={cfg['min_reading']} min_words={cfg['min_words']} max_fallback_fraction={cfg['max_fallback_fraction']}"
            )
...
def main():
    ...
    ap.add_argument(
        "--max-fallback-fraction",
        type=float,
        default=0.35,
        help="Maximum fraction of accepted examples allowed to be fallback/multistage before capping.",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="Run filtering and stats without DB inserts / embedding calls (embeddings mocked).",
    )
    ap.add_argument(
        "--min-accepted",
        type=int,
        help="Target minimum accepted examples; relax constraints iteratively if not reached.",
    )
    ap.add_argument(
        "--relax-step",
        type=float,
        default=8.0,
        help="Relaxation delta for readability (and scaled adjustments) per attempt when below min-accepted.",
    )
    ap.add_argument(
        "--sidecar-dir",
        type=str,
        help="Directory to write per-example enriched meta sidecar files or aggregate JSON.",
    )
    ap.add_argument(
        "--sidecar-mode",
        type=str,
        choices=["files", "aggregate"],
        default="files",
        help="Sidecar write strategy.",
    )
    ap.add_argument(
        "--manifest-out",
        type=str,
        help="Path to write batch ingestion manifest JSON summarizing quality + config.",
    )
    args = ap.parse_args()
    summary = process(
        file=args.file,
