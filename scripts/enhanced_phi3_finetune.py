            ...
            if "[TASK]" in instruction:
                ...
                if len(parts) > 1:
                    formatted_instruction = (
                        parts[1].split("[TEACHING_RESPONSE]")[0].strip()
                    )
                else:
                    formatted_instruction = instruction
            else:
...
def main():
    ...
    parser.add_argument(
        "--train-file", required=True, help="Training data file (JSONL)"
    )
    ...
    parser.add_argument(
        "--output-dir", default="models/phi3-enhanced-tutor", help="Output directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-steps", type=int, default=250, help="Save checkpoint every N steps"
    )
    parser.add_argument(
    ...
    logger.info("Starting enhanced Phi-3 fine-tuning")
