...
def insert(rows):  # legacy path not currently used (kept for backward compat)
    ...
    with psycopg2.connect(DSN) as conn:
        with conn.cursor() as cur:
            execute_values(
                cur,
                "INSERT INTO doc_embeddings (source, chunk, embedding, batch_tag, content_hash, metadata) VALUES %s",
                rows,
            )
...
def _insert_clarifying(
    ...
    with psycopg2.connect(DSN) as conn:
        with conn.cursor() as cur:
            execute_values(
                cur,
                "INSERT INTO chunk_clarifying_questions (chunk_id, question, priority, metadata) VALUES %s ON CONFLICT DO NOTHING",
                out_records,
            )
    print(
...
def process(
    ...
    with psycopg2.connect(DSN) as conn:
        with conn.cursor() as cur:
            # Use temp table approach for RETURNING efficiency if large; for now rely on RETURNING with execute_values fetch
            insert_sql = "INSERT INTO doc_embeddings (source, chunk, embedding, batch_tag, content_hash, metadata) VALUES %s RETURNING id, chunk"
            execute_values(cur, insert_sql, all_rows, fetch=True)
            for rid, chunk_text_value in cur.fetchall():
                inserted_chunks.append((rid, chunk_text_value))
        # After commit insertion, optionally create clarifying questions
        if gen_clarifying > 0:
            _insert_clarifying(
                inserted_chunks,
                gen_clarifying,
