...
class MathModelComparator:
    ...
    def compare_math_models(self, test_problem):
        ...
        models = {"General Math": math_response, "Tutoring": tutoring_response}
        ...
        for model_name, response in models.items():
            ...
            for category, score in category_scores.items():
                print(
                    f"  {category.replace('_', ' ').title()}: {score}/{len(tutoring_indicators[category])}"
                )

        # Determine which model demonstrates better tutoring
        math_score = sum(
            1
            for indicators in tutoring_indicators.values()
            for indicator in indicators
            if indicator in math_response.lower()
        )
        tutoring_score = sum(
            1
            for indicators in tutoring_indicators.values()
            for indicator in indicators
            if indicator in tutoring_response.lower()
        )
