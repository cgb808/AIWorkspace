...
def migrate_files():
    ...
    def safe_move(source, destination, description):
        ...
        try:
            ...
            if source_path.exists():
                ...
                print(f"✅ Moved: {source_path.name} → {dest_path.relative_to(workspace)}")
    ...
    manifest_files = list(workspace.glob("data/MANIFEST_*.json")) + list(workspace.glob("data/SUMMARY_*.txt"))
...
def main():
    ...
    print(f"\n🎉 MIGRATION COMPLETE!")
    print("=" * 40)
    print(f"✅ Files migrated to organized structure")
