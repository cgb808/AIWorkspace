...
class MathematicsModelsComparator:
    ...
    def compare_mathematics_models(self, test_problem):
        ...
        teaching_indicators = {
            ...
            "pedagogical_approach": ["notice", "remember", "important", "careful", "think about"],
            "methodology_focus": ["approach", "strategy", "method", "technique", "process"],
            "student_engagement": ["you", "your", "can you", "what do you", "let's work"],
            "clarity_indicators": ["clear", "step-by-step", "systematic", "organized", "logical"]
        }
        ...
        for model in models_to_test:
            ...
            for category, score in category_scores.items():
                print(f"  {category.replace('_', ' ').title()}: {score}/{len(teaching_indicators[category])}")
        ...
        print(f"   Pure Methodology Focus: {'✅ Highest' if methodology_score == max(methodology_score, tutoring_score, general_score) else '📊 Moderate' if methodology_score > min(methodology_score, tutoring_score, general_score) else '❌ Lowest'}")
        print(f"   Tutoring Integration: {'✅ Optimal' if tutoring_score >= methodology_score else '📊 Good' if tutoring_score > general_score else '❌ Needs Work'}")
        print(f"   Mathematical Content: {'✅ Strong' if general_score > 0 else '❌ Weak'}")
