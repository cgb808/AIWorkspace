...
SENSITIVE_MARKERS = [
    "KEY", "SECRET", "PASS", "TOKEN", "PASSWORD", "PAT", "API_KEY", "CLIENT_SECRET"
]
...
def redact_value(name: str, value: str, prefix_chars: int, paranoid: bool) -> str:
    if name == "DATABASE_URL":
        ...
        m = re.match(r"^(?P<scheme>\w+?)://(?:(?P<user>[^:@/]+)(?::(?P<pw>[^@/]*))?@)?(?P<rest>.*)$", value)
        if m:
...
def process_lines(lines: Iterable[str], strict: bool, prefix: int, paranoid: bool) -> Iterable[str]:
    for line in lines:
        raw = line.rstrip('\n')
...
def main():
    ...
    ap.add_argument('--input', '-i', default='.env', help='Input .env file or - for stdin')
    ap.add_argument('--output', '-o', default='.env.redacted', help='Output file (or - for stdout)')
    ap.add_argument('--strict', action='store_true', help='Redact even SUPABASE_URL')
    ap.add_argument('--prefix', type=int, default=0, help='Show first N chars before redaction marker')
    ap.add_argument('--paranoid', action='store_true', help='Also mask database username in DATABASE_URL')
    args = ap.parse_args()
