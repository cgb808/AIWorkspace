...
def main():
    ...
    if not cloud:
        print(
            "ERROR: Set one of SUPABASE_DB_URL / SUPABASE_DIRECT_URL / SUPABASE_POOLER_URL / DATABASE_URL to cloud DSN",
            file=sys.stderr,
        )
        return 2
    local = os.getenv(
        "LOCAL_DSN", "postgresql://postgres:postgres@localhost:5432/rag_db"
    )
    ...
    cmd = [
        sys.executable,
        "scripts/transfer_tables_between_dbs.py",
        "--source",
        cloud,
        "--target",
        local,
