...
import logging
import os
from app.personas import resolve_persona, PERSONAS, DEFAULT_PERSONA_KEY
...
from app.rag.ranking_router import rag_query2, Query2Payload  # reuse logic for legacy endpoint refactor
from app.audio import transcription_router
from app.audio import tts_router
...
from app.rag.streaming_router import router as streaming_router  # SSE phased diff (Artifact D)
...
try:
    ...
    log("static mounts complete", assets_dir=assets_dir, assets_present=os.path.isdir(assets_dir))
except Exception:
...
@app.post("/rag/query")  # Refactored legacy endpoint: delegates scoring to /rag/query2 logic; now supports backend preference
async def rag_query(request: Request) -> Dict[str, Any]:
    ...
    if not answer:
        ...
        base_fallback = "I can still answer from general knowledge, but retrieval is temporarily unavailable." if retrieval_error else ""
