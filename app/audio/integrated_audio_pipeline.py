...
class WhisperCPPTranscriber:
    ...
    def __init__(self):
        ...
        self.model_path = os.path.join(self.whisper_base, "models", f"ggml-{self.whisper_model}.bin")
...
<|assistant|>"""
    ...
    def _fallback_classify(self, user_input: str) -> ToolClassification:
        ...
        if any(word in user_lower for word in ["solve", "calculate", "graph", "equation", "math"]):
            return ToolClassification(
        ...
        elif any(word in user_lower for word in ["wait", "pause", "stop", "louder", "voice"]):
            return ToolClassification(
                primary_category="AUDIO",
                tools_needed=["audio_pause"] if "pause" in user_lower else ["tts_speak"],
                specialist_required="phi3_conversation_handler",
                confidence=0.90,
                voice_preference="amy",
                parameters={"action_type": "pause" if "pause" in user_lower else "general"}
            )
        ...
        # Search keywords
        elif any(word in user_lower for word in ["what is", "tell me", "find", "search"]):
            return ToolClassification(
...
class Phi3SpecialistOrchestrator:
    ...
    def _load_math_specialist(self):
        ...
        return MockPhi3Specialist("mathematics", "I'm your math tutor, trained on Socratic questioning.")
    
    def _load_conversation_specialist(self):
        """Load conversation handling specialist (trained on our 71 interruption examples)."""
        return MockPhi3Specialist("conversation", "I handle graceful conversation flow and interruptions.")
    
    def _load_visual_specialist(self):
        """Load visual explanation specialist.""" 
        return MockPhi3Specialist("visual", "I create diagrams and visual explanations.")
    
    def _load_knowledge_specialist(self):
        """Load knowledge retrieval specialist."""
        return MockPhi3Specialist("knowledge", "I search and synthesize knowledge from your RAG system.")
    
    def _load_workflow_specialist(self):
        """Load workflow orchestration specialist."""
        return MockPhi3Specialist("workflow", "I manage multi-step processes and workflows.")
...
class MockPhi3Specialist:
    ...
    async def generate_response(self, 
        ...
        if self.domain == "mathematics":
            ...
            tool_calls = [{"tool": tool, "parameters": {}} for tool in tool_context.tools_needed]
        
        else:
            response_text = f"As your {self.domain} specialist, I'll help with: {user_input}"
            tool_calls = [{"tool": tool, "parameters": {}} for tool in tool_context.tools_needed]
...
class PiperTTSEngine:
    ...
    def __init__(self):
        ...
        self.voice_models = {
            ...
            "jarvis": os.getenv("PIPER_JARVIS_MODEL", "models/piper/en_GB-alan-low.onnx"),
            "alan": os.getenv("PIPER_ALAN_MODEL", "models/piper/en_GB-alan-low.onnx"),
            "southern_male": os.getenv("PIPER_SOUTHERN_MALE_MODEL", "models/piper/en_GB-southern_english_male-low.onnx")
        }
...
class IntegratedAudioPipeline:
    ...
    async def process_audio(self, audio_data: bytes) -> AudioProcessingResult:
        ...
        logger.info(f"Tool classification: {tool_classification.primary_category}, confidence: {tool_classification.confidence}")
        
        # 3. Specialist Response (Phi-3)
        specialist_response = await self.phi3_orchestrator.get_response(
            tool_classification.specialist_required,
            transcription,
            tool_classification
        )
        ...
        # 4. Tool Execution
        tool_outputs = await self.tool_executor.execute_tools(specialist_response["tool_calls"])
        ...
        audio_output = await self.piper.synthesize(
            ...
            voice=tool_classification.voice_preference
        )
