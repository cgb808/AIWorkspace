...
async def transcribe(file: UploadFile = File(...), language: Optional[str] = None):
    if not os.path.exists(BINARY_PATH):
        raise HTTPException(status_code=500, detail="whisper.cpp binary not built (run setup script)")
    ...
    if proc.returncode != 0:
        raise HTTPException(status_code=500, detail=f"whisper.cpp failed: {proc.stderr[-400:]}")
