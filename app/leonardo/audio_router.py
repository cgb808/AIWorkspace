...
Please provide a thoughtful, well-reasoned response that demonstrates analytical thinking. Consider multiple perspectives, provide clear explanations, and offer educational insights where appropriate."""
        ...
        if not response_text:
            raise HTTPException(status_code=500, detail="Leonardo did not generate a response")
...
async def leonardo_status():
    ...
    try:
        ...
        tts_available = subprocess.run(
            ["which", "piper"],
            capture_output=True
        ).returncode == 0
        
        # Test Whisper availability  
        whisper_available = subprocess.run(
            ["which", "whisper"],
            capture_output=True
        ).returncode == 0
        ...
        return {
            ...
            "status": "ready" if all([leonardo_available, tts_available, whisper_available]) else "partial",
            "capabilities": {
                "analytical_thinking": leonardo_available,
                "text_to_speech": tts_available,
                "speech_to_text": whisper_available,
                "voice_interaction": leonardo_available and tts_available and whisper_available
            }
        }
