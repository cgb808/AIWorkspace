...
class RAGPipeline:
	def __init__(self, db_client: SupportsVectorSearch, embedder: SupportsEmbed, llm_client: SupportsLLM):
	...
	def _derive_score(self, item: Dict[str, Any]) -> float:
		...
		if ts:
			...
			half_life = float(os.getenv("RAG_RECENCY_HALF_LIFE", "604800"))  # 7 days seconds
			age = max(0.0, (float(os.getenv("CURRENT_TIME_OVERRIDE", "0")) or 0) - float(ts)) if os.getenv("CURRENT_TIME_OVERRIDE") else 0.0
	...
	def retrieve_context(self, embedding: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
