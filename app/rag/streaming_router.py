...
from .feature_assembler import assemble_features, Candidate
...
async def _iter_events(payload: StreamQueryPayload) -> AsyncGenerator[str, None]:
    ...
    for pref in mem_prefixes:
        if lower_q.startswith(pref):
            ...
            if remainder:
                ...
                try:
                    ...
                    with db.conn.cursor() as cur:  # type: ignore
                        cur.execute(
                            ...
                            RETURNING id
                            """,
                            ("memory", remainder, emb, json.dumps({
                                "source": "memory",
                                "wake": True,
                ...
                yield "data: " + json.dumps({"phase": "MEM", "ack": False, "error": "no content"}) + "\n\n"
                return
    ...
    base_hits, meta = _ann_search(q_vec, top_k, payload.query, table="doc_embeddings", exclude_sources=["memory"])
    meta["index"] = _choose_index(q_vec, payload.query)
    raw_hits = list(base_hits)
    if enable_memory:
        mem_hits, meta_mem = _ann_search(q_vec, mem_top, payload.query, table="doc_embeddings", sources=["memory"])
        for mh in mem_hits:
    ...
    p0_results = [
        {"id": h["id"], "text_preview": (h["text"] or "")[:160], "distance": h["distance"]}
        for h in raw_hits
    ]
    ev0 = {"phase": "P0", "provisional": True, "results": p0_results, "meta": {**meta, "embed_ms": embed_ms}}
    yield "data: " + json.dumps(ev0) + "\n\n"
    await asyncio.sleep(0)  # cooperative yield

    # ---- Phase P1 (LTR) ----
    t_ltr_start = time.time()
    candidates = [Candidate(chunk_id=h["id"], text=h["text"], distance=h["distance"], source=h.get("source")) for h in raw_hits]
    ...
    for i, r in enumerate(enriched):
        ...
        if old is None:
            deltas_p1.append({"id": r["id"], "change": "new", "new_rank": i, "score": r["score"]})
        else:
            if old != i:
                deltas_p1.append({
                    "id": r["id"],
    ...
    ev1 = {"phase": "P1", "provisional": False, "feature_names": names, "delta": deltas_p1, "ltr_ms": ltr_ms, "result_count": len(enriched)}
    ...
    for i, r in enumerate(adjusted):
        ...
        if old is None:
            deltas_p2.append({"id": r["id"], "change": "new", "new_rank": i, "score": r["score"]})
        else:
    ...
    ev2 = {"phase": "P2", "provisional": False, "delta": deltas_p2, "final": True, "result_count": len(adjusted)}
