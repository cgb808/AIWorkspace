...
class ToolControlDatasetGenerator:
    def __init__(self):
        self.tool_categories = {
            "MATHEMATICAL": {
                "tools": ["calculator", "graphing", "latex_renderer", "equation_solver", "formula_display"],
                "specialist": "phi3_mathematics_tutor",
                "keywords": ["solve", "calculate", "graph", "equation", "formula", "math", "algebra", "calculus", "geometry"],
                "voice_preference": "jarvis"  # Analytical voice for math
            },
            "VISUAL": {
                "tools": ["canvas_draw", "diagram_create", "chart_generate", "latex_display", "image_annotate"],
                "specialist": "phi3_visual_explainer",
                "keywords": ["show", "draw", "diagram", "chart", "visualize", "illustrate", "display", "picture"],
                "voice_preference": "alan"  # Clear voice for explanations
            },
            "AUDIO": {
                "tools": ["tts_speak", "voice_change", "audio_pause", "audio_resume", "volume_adjust"],
                "specialist": "phi3_conversation_handler", 
                "keywords": ["wait", "pause", "stop", "continue", "louder", "quieter", "repeat", "speak"],
                "voice_preference": "amy"  # Default conversational voice
            },
            "SEARCH": {
                "tools": ["rag_search", "vector_query", "context_retrieve", "memory_access", "knowledge_lookup"],
                "specialist": "phi3_knowledge_retriever",
                "keywords": ["find", "search", "lookup", "remember", "what is", "tell me about", "explain"],
                "voice_preference": "jarvis"  # Authoritative for facts
            },
            "WORKFLOW": {
                "tools": ["task_sequence", "multi_step", "context_switch", "session_manage", "progress_track"],
                "specialist": "phi3_workflow_orchestrator",
                "keywords": ["next", "then", "after", "sequence", "steps", "workflow", "process", "continue"],
    ...
    def generate_mathematical_examples(self, count: int = 1000) -> List[Dict]:
        ...
        # Basic calculations
        calculations = [
            "What's 15 * 23?", "Calculate the square root of 144", "What's 2^8?",
            "Find 25% of 400", "What's the area of a circle with radius 5?",
            "Convert 75 degrees Fahrenheit to Celsius"
        ]
        
        # Equation solving
        equations = [
            "Solve x^2 + 5x - 6 = 0", "Find x when 2x + 7 = 15", 
            "Solve the system: x + y = 5, 2x - y = 1", "Factor x^2 - 9",
            "Solve for y: 3y - 4 = 2y + 8", "Find the roots of x^2 - 4x + 3 = 0"
        ]
        
        # Graphing requests
        graphs = [
            "Graph the function y = x^2 + 2x - 3", "Plot y = sin(x)", 
            "Show me the graph of y = 2x + 1", "Graph the parabola y = -x^2 + 4",
            "Plot the exponential function y = 2^x", "Graph y = |x - 2|"
        ]
        
        # LaTeX rendering
        latex_requests = [
            "Show me the quadratic formula", "Display the Pythagorean theorem",
            "Write out the derivative of x^3", "Show me the binomial theorem",
            "Display Einstein's mass-energy equation", "Show the formula for compound interest"
        ]
        ...
        for i in range(count):
            ...
            if any(word in user_input.lower() for word in ["calculate", "what's", "find", "convert"]):
                tools_needed = ["calculator"]
                if "area" in user_input or "formula" in user_input:
                    tools_needed.append("latex_renderer")
            elif any(word in user_input.lower() for word in ["solve", "factor", "roots"]):
            ...
            elif any(word in user_input.lower() for word in ["show", "display", "write"]):
    ...
    def generate_audio_control_examples(self, count: int = 500) -> List[Dict]:
        ...
        interruptions = [
            "Wait", "Hold on", "Stop", "Pause", "Let me think",
            "Wait a minute", "Hold on a second", "Actually, wait",
            "Can you pause?", "Stop talking for a moment", "Give me a second"
        ]
        
        # Volume/speed control
        audio_controls = [
            "Speak louder", "Can you speak quieter?", "Slow down",
            "Talk faster", "Lower your voice", "Increase volume",
            "Speak more clearly", "Can you repeat that?", "Say that again"
        ]
        
        # Voice change requests
        voice_changes = [
            "Change your voice", "Use a different voice", "Switch to jarvis voice",
            "Can you use the British voice?", "Use the southern accent",
            "Switch to the female voice", "Use the male voice"
        ]
        
        # Continuation commands
        continuations = [
            "Continue", "Go on", "Keep going", "What's next?",
            "Proceed", "Continue where you left off", "Resume",
            "Keep talking", "Go ahead", "Carry on"
        ]
        
        all_audio_inputs = interruptions + audio_controls + voice_changes + continuations
        
        for i in range(count):
            user_input = random.choice(all_audio_inputs)
            
            # Determine specific audio tools needed
            if any(word in user_input.lower() for word in ["wait", "hold", "stop", "pause"]):
                tools_needed = ["audio_pause"]
                action_type = "pause"
            elif any(word in user_input.lower() for word in ["louder", "quieter", "volume"]):
                tools_needed = ["volume_adjust"]
                action_type = "volume_control"
            elif any(word in user_input.lower() for word in ["voice", "accent", "british", "jarvis"]):
                tools_needed = ["voice_change"]
                action_type = "voice_selection"
            elif any(word in user_input.lower() for word in ["continue", "resume", "go on"]):
    ...
    def generate_visual_examples(self, count: int = 800) -> List[Dict]:
        ...
        # Drawing requests
        drawings = [
            "Draw a triangle", "Show me a circle", "Create a diagram of a cell",
            "Draw the water cycle", "Illustrate a food chain", "Draw a house",
            "Show me a family tree", "Create a flowchart", "Draw a coordinate plane"
        ]
        
        # Chart/graph requests (different from mathematical graphing)
        charts = [
            "Create a pie chart", "Make a bar graph", "Show me a timeline",
            "Create a Venn diagram", "Make a scatter plot", "Show data in a table",
            "Create an organizational chart", "Make a comparison chart"
        ]
        
        # Annotation requests
        annotations = [
            "Label this diagram", "Add arrows to show the flow", "Highlight the important parts",
            "Mark the key points", "Add text to explain", "Show the steps visually",
            "Point out the differences", "Circle the main idea"
        ]
        
        # Display requests
        displays = [
            "Show me an image of the solar system", "Display the periodic table",
            "Show a map of Europe", "Display the human skeleton", "Show me a world map",
            "Display the parts of a flower", "Show the layers of the Earth"
        ]
        ...
        for i in range(count):
            ...
            elif any(word in user_input.lower() for word in ["label", "arrow", "highlight", "mark"]):
    ...
    def generate_search_examples(self, count: int = 700) -> List[Dict]:
        ...
        # Knowledge queries
        knowledge = [
            "What is photosynthesis?", "Tell me about the American Revolution",
            "Explain quantum mechanics", "What are black holes?", "Define democracy",
            "How does DNA work?", "What is machine learning?", "Explain gravity"
        ]
        
        # Memory/context retrieval
        memory = [
            "What did we discuss yesterday?", "Remember our last conversation about math?",
            "What was that formula we used before?", "Recall what I learned about history",
            "What topics have we covered?", "Show me my progress", "What did I struggle with?"
        ]
        
        # Search requests
        searches = [
            "Find information about climate change", "Look up the capital of France",
            "Search for examples of metaphors", "Find the definition of osmosis",
            "Look up recent news about space", "Search for primary sources",
        ...
        for i in range(count):
            ...
            if any(word in user_input.lower() for word in ["remember", "recall", "discussed", "conversation"]):
                tools_needed = ["memory_access", "context_retrieve"]
                search_type = "memory"
            elif any(word in user_input.lower() for word in ["find", "search", "look up"]):
    ...
    def generate_workflow_examples(self, count: int = 500) -> List[Dict]:
        ...
        processes = [
            "Let's work through this step by step", "What's the next step?",
            "Take me through the process", "Show me the sequence",
            "What comes after this?", "Let's continue with the next part",
            "Walk me through this procedure", "What's the workflow here?"
        ]
        
        # Context switching
        switches = [
            "Let's switch to a different topic", "Can we change subjects?",
            "Move on to the next lesson", "Let's talk about something else",
            "Switch to math now", "Change to history mode",
            "Let's do science instead", "Can we work on writing?"
        ]
        
        # Session management
        sessions = [
            "Save my progress", "Remember where we left off", "Start a new session",
            "End this lesson", "Take a break", "Resume our session",
            "Track my learning", "Show my achievements", "What's my status?"
        ]
        ...
        for i in range(count):
            ...
            if any(word in user_input.lower() for word in ["step", "sequence", "process", "workflow"]):
                tools_needed = ["task_sequence", "multi_step"]
                workflow_type = "process"
            elif any(word in user_input.lower() for word in ["switch", "change", "move", "different"]):
                tools_needed = ["context_switch"]
                workflow_type = "context_switch"
            elif any(word in user_input.lower() for word in ["save", "progress", "session", "track"]):
    ...
    def _determine_visual_type(self, user_input: str) -> str:
        ...
        if any(word in user_input.lower() for word in ["chart", "graph", "plot"]):
            return "chart"
        elif any(word in user_input.lower() for word in ["diagram", "flowchart", "tree"]):
            return "diagram"
    ...
    def generate_mixed_examples(self, count: int = 500) -> List[Dict]:
        ...
        for i in range(count):
            ...
            elif any(word in user_input.lower() for word in ["find", "search", "information"]):
                primary = "SEARCH"
                specialist = "phi3_knowledge_retriever"
                tools = ["rag_search", "vector_query"]
            elif any(word in user_input.lower() for word in ["show", "diagram", "graph"]):
                primary = "VISUAL" 
                specialist = "phi3_visual_explainer"
                tools = ["diagram_create", "chart_generate"]
            elif any(word in user_input.lower() for word in ["step", "process", "workflow"]):
                primary = "WORKFLOW"
                specialist = "phi3_workflow_orchestrator"
            ...
            example = {
                ...
                "tool_classification": {
                    ...
                    "confidence": round(random.uniform(0.75, 0.90), 2),  # Lower confidence for mixed
                    "voice_preference": self.tool_categories[primary]["voice_preference"],
                    "parameters": {
                        "complexity": "high",
                        "multi_category": True
                    }
