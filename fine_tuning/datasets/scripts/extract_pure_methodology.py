...
def extract_pure_methodology():
    ...
    hybrid_file = workspace / "fine_tuning/datasets/processed/hybrid/hybrid_methodology_math_dataset.jsonl"
    ...
    print(f"Total Examples: {len(pure_methodology_examples) + len(math_with_methodology_examples)}")
    
    # Save pure methodology examples
    pure_methodology_file = workspace / "fine_tuning/datasets/processed/pure_methodology_tutoring.jsonl"
    ...
    for i, example in enumerate(pure_methodology_examples[:5]):
        print(f"\nExample {i+1}:")
        instruction = example.get('instruction', '')[:100] + "..." if len(example.get('instruction', '')) > 100 else example.get('instruction', '')
        output = example.get('output', '')[:150] + "..." if len(example.get('output', '')) > 150 else example.get('output', '')
    ...
    print("=" * 40)
    for methodology, count in sorted(methodology_breakdown.items(), key=lambda x: x[1], reverse=True):
        print(f"  {methodology}: {count} examples")
    
    # Save breakdown
    breakdown_file = workspace / "fine_tuning/datasets/processed/pure_methodology_breakdown.json"
...
def main():
    ...
    if pure_examples:
        ...
        print(f"✅ Ready for base model training")
        print(f"✅ Can be combined with personality data")
