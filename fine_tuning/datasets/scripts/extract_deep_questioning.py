...
class DeepSocraticExtractor:
    ...
    def __init__(self, workspace_root="/home/cgbowen/AIWorkspace"):
        ...
        self.progressive_indicators = [
            "first", "then", "next", "building on that", "going deeper",
            "let's dig into", "taking it further", "now consider",
            "following that logic", "expanding on"
        ]
    ...
    def extract_deep_questioning_sequences(self):
        ...
        socratic_file = self.fine_tuning_dir / "datasets/processed/socratic_method/socratic_method_high_confidence.jsonl"
        
        if not socratic_file.exists():
            print("❌ Socratic dataset not found. Run find_socratic_method_datasets.py first.")
            return []
                ...
                try:
                    ...
                    if self._has_deep_questioning(example):
                        ...
                        example['deep_questioning_metadata'] = {
                            'drill_down_score': questioning_analysis['drill_down_score'],
                            'intent_probing_score': questioning_analysis['intent_probing_score'],
                            'question_sequence_length': questioning_analysis['question_sequence_length'],
                            'questioning_techniques': questioning_analysis['techniques'],
    ...
    def _has_deep_questioning(self, example):
        ...
        drill_down_matches = sum(1 for pattern in self.drill_down_patterns 
                                if re.search(pattern, combined_text))
        
        # Check for intent probing
        intent_matches = sum(1 for pattern in self.intent_probing_patterns 
                           if re.search(pattern, combined_text))
        
        # Check for progressive questioning
        progressive_matches = sum(1 for indicator in self.progressive_indicators 
                                if indicator in combined_text)
        
        # Must have at least 2 drill-down patterns or 1 intent probing pattern
        return drill_down_matches >= 2 or intent_matches >= 1 or progressive_matches >= 1
    ...
    def _analyze_questioning_depth(self, example):
        ...
        drill_down_score = sum(1 for pattern in self.drill_down_patterns 
                              if re.search(pattern, combined_text))
        
        # Count intent probing patterns
        intent_probing_score = sum(1 for pattern in self.intent_probing_patterns 
                                  if re.search(pattern, combined_text))
    ...
    def create_questioning_mastery_dataset(self, examples):
        ...
        expert_examples = [ex for ex in examples if ex['deep_questioning_metadata']['depth_level'] == 'expert_level']
        advanced_examples = [ex for ex in examples if ex['deep_questioning_metadata']['depth_level'] == 'advanced']
        intermediate_examples = [ex for ex in examples if ex['deep_questioning_metadata']['depth_level'] == 'intermediate']
            ...
            if technique_examples:
                ...
                with open(technique_file, 'w') as f:
                    for example in technique_examples:
                        f.write(json.dumps(example) + '\n')
                print(f"📋 {technique.replace('_', ' ').title()}: {len(technique_examples)} examples")
    ...
    def _create_questioning_analysis_report(self, examples, output_dir):
        ...
        report = {
            ...
            "average_drill_down_score": sum(ex['deep_questioning_metadata']['drill_down_score'] for ex in examples) / len(examples),
            "average_intent_probing_score": sum(ex['deep_questioning_metadata']['intent_probing_score'] for ex in examples) / len(examples),
            "questioning_mastery_ready": len([ex for ex in examples if ex['deep_questioning_metadata']['depth_level'] in ['expert_level', 'advanced']]),
            "sample_expert_examples": [ex for ex in examples if ex['deep_questioning_metadata']['depth_level'] == 'expert_level'][:5]
        }
    ...
    def show_sample_deep_questioning(self, examples, num_samples=5):
        ...
        for i, example in enumerate(expert_examples[:num_samples], 1):
            ...
            instruction = example.get('instruction', '')[:150] + "..." if len(example.get('instruction', '')) > 150 else example.get('instruction', '')
            output = example.get('output', '')[:300] + "..." if len(example.get('output', '')) > 300 else example.get('output', '')
