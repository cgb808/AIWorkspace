...
def main():
    ...
    ap.add_argument('--dir', default='fine_tuning/datasets/processed/interruption_handling')
    ap.add_argument('--out', default='fine_tuning/datasets/processed/interruption_handling/interruption_all_merged.jsonl')
    args = ap.parse_args()
    ...
    print(f"[merge] wrote {args.out} records={len(merged)} sources_scanned={len(PRIMARY_FILES)}")
