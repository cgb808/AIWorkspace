...
class HybridModelTrainer:
    ...
    def load_hybrid_dataset(self):
        ...
        pure_methodology_count = len([ex for ex in examples if ex.get('example_type') == 'pure_methodology'])
        math_methodology_count = len([ex for ex in examples if ex.get('example_type') == 'mathematics_with_methodology'])
    ...
    def format_hybrid_example(self, example):
        ...
        if example_type == 'pure_methodology':
            formatted_prompt = f"[HYBRID_TRAINING_MODE] [PURE_METHODOLOGY] {instruction}"
    ...
    def simulate_hybrid_training(self, examples, phases=3):
        ...
        print(f"Approach: 50% pure methodology + 50% math + methodology")
        ...
        training_log = {
            ...
            "dataset_composition": {
                "pure_methodology": len([ex for ex in examples if ex.get('example_type') == 'pure_methodology']),
                "mathematics_with_methodology": len([ex for ex in examples if ex.get('example_type') == 'mathematics_with_methodology'])
            },
            "phases": {},
        ...
        for phase_num, config in enumerate(phase_configs, 1):
            ...
            for i, example in enumerate(phase_examples[:3]):
                formatted_prompt, expected_response = self.format_hybrid_example(example)
                
                print(f"  Training example {i+1} ({example.get('example_type', 'unknown')})...")
...
def main():
    ...
    try:
        ...
        print(f"   Pure methodology: {training_log['dataset_composition']['pure_methodology']}")
        print(f"   Math + methodology: {training_log['dataset_composition']['mathematics_with_methodology']}")
        print(f"   Status: {training_log['status']}")
        print(f"   Ready for comparison testing!")
